...
function readPrecoMult() {
    async function fetchAsync() {
        ...
        const response = await fetch(`hhttps://raftminho.herokuapp.com/activities/${idAct}`);
...
function readPrecoCartao() {
    async function fetchAsync() {
        ...
        const response = await fetch(`https://raftminho.herokuapp.com/activities/${idAct}`);
        const acts = await response.json();
...
botaoSubmeter.addEventListener("click", async (event) => {
    ...
    fetch(`https://raftminho.herokuapp.com/teams/${idTeam}/registers/${idAct}`, {
        headers: {
